...
**Veo watermark removal.** The desktop hero-loop mp4 had Google's "Veo" generation watermark baked into the bottom-right corner — small white text the old full-bleed vignette used to hide, but which the new editorial frame exposed. Cleaned via `ffmpeg -i public/hero-loop.mp4 -vf "delogo=x=1165:y=676:w=70:h=28" -c:v libx264 -b:v 1300k -maxrate 1700k -bufsize 2600k -pass {1,2} -preset slow -movflags +faststart -an public/hero-loop.mp4` (two-pass to match original bitrate of ~1.2 Mbps; final size 7.7MB vs 7.4MB original — within 5%). The delogo filter inpaints the watermark area from surrounding pixels; against the busy foliage backdrop the residual smudge is barely perceptible at 1× viewing scale. The original mp4 is preserved as `public/hero-loop.original-with-veo.mp4` in case the source ever needs to be re-derived. Mobile mp4 was sampled at 5 timestamps and confirmed watermark-free — no edit needed there. As belt-and-suspenders, added a subtle bottom-right radial-gradient overlay inside the frame container (200×90px, navy-toned, fading to transparent) that washes any residual artifact while reading as part of the frame's editorial chrome.
