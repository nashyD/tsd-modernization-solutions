...
| **Phone** | +1 (980) 227-9003 (Vapi-managed; migrated from Telnyx (704) 317-5630 on 2026-05-05 because Vapi's BYO-Telnyx integration silently force-injects SIP REFER on transferCall, which Telnyx rejected during outbound dial — see PROJECT_LOG 2026-05-05 for the full diagnostic. Prior history: (704) 317-5630 was Telnyx-owned starting 2026-04-28, replacing the brief (704) 741-1746 Twilio line, which itself replaced the (704) 275-1410 GV line on 2026-04-27.) Founders also have individual TSD work lines on Vapi-managed numbers: Nash `+1 (980) 227-4913`, Grant `+1 (980) 227-4917`, Bishop `+1 (980) 227-4702`. |
...
| Voice routing | Vapi-managed (PSTN inbound + outbound + transferCall) | TSD's published company number is `+1 (980) 227-9003` (Vapi-managed since 2026-05-05). Founders' TSD work lines (`+19802274913` / `+19802274917` / `+19802274702`) are also Vapi-managed and bound to per-founder Vapi assistants in the `tsd-receptionist` repo. The earlier voice-receptionist Fly app (Pipecat-on-Fly) was decommissioned when the Telnyx number it relied on was migrated; the Telnyx account is retained for SBO client deploys where the buyer-owns-the-number handoff story matters. Replaced the BYO-Telnyx + Vapi pairing on 2026-05-05 because BYO-Telnyx silently injected SIP REFER on transferCall, which Telnyx rejected — see PROJECT_LOG 2026-05-05 entry. |
...
| Repo | Purpose | Stack |
|---|---|---|
| [`tsd-modernization-app`](https://github.com/nashyD/tsd-modernization-solutions/tree/main/tsd-modernization-app) (sibling subdir of this repo) | Public `/audit` lead-magnet + authenticated `/app` client portal + `/admin` surface, fronted onto `tsd-modernization.com` via path-based rewrites in this site's `vercel.json` | Next.js 16 (App Router) + TypeScript + Tailwind v4. Supabase Postgres + Auth (magic link) + Storage. Anthropic Claude Sonnet 4.6 with tool-use forcing for strict-schema audit synthesis. Google Places (New API) + `cheerio` for V1 scraping (no Playwright until Phase 4 monthly snapshots). Resend for transactional email. Upstash for rate-limit. Deployed as its own Vercel project at `tsd-modernization-solutions-uc71.vercel.app`; this repo's other Vercel project (the marketing Vite site) doesn't see or build the new subdir because its Root Directory is `tsd-modernization/`. |
...
7. ~~**Personal-line dual-persona Austin** — second Telnyx number (`+1 704 317 5534`) provisioned for the founder's personal line use case; persona-dispatch code in voice-receptionist still to ship.~~ **Superseded** 2026-05-05 by per-founder Vapi assistants (`tsd-personal`, `grant-personal`, `bishop-personal` in `tsd-receptionist`). The dual-persona-on-one-number approach is no longer needed — each founder has their own Vapi-managed line + assistant.
