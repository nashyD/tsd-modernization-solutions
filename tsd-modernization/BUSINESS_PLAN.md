...
**2026-06-27 conversion + voice pass (improvement-loop cycle 5, presentational):** A loop batch tightened the funnel and enforced the voice rules with no change to the offer, pricing, packaging, or positioning substance. The hero **subhead was rewritten** from a run-on into three clean sentences; it keeps the $540/mo bakery proof and "managed by us or owned by you" but **drops** "with the same AI the big players run" and the redundant audience clause, so the 2026-06-21 note's quote of those two phrases is now stale (the empowerment H1 and the promise-then-proof discipline are unchanged). Mobile "Book a fit call" now stays visible in the nav on phones (was hidden behind the Menu). The `/book` page gained a trust strip (free 30-min call, a cost-cut audit you keep, 48-hour proposal, money-back) and a Calendly-failure fallback. Site-wide, the no-em-dash voice rule is now enforced across all rendered and meta copy (168 em dashes removed; en-dash number ranges and compound hyphens preserved). Each core page emits a single SEO `<h1>`. All build- and browser-verified, deployed to prod 2026-06-27. Code: `src/Layout.jsx`, `src/pages/Home.jsx`, `src/pages/Book.jsx`, `src/shared.jsx`, plus a copy sweep across page/data/meta/JSON-LD files. See PROJECT_LOG 2026-06-27 "loop cycles 2-5." §4 + §8.2 unchanged in substance.

**2026-06-21 empowerment layer (positioning):** The homepage hero now LEADS with an empowerment promise and keeps the money-saver pitch as the proof beneath it. New H1: *"We find the leak. You outrun the franchise."* The subhead carries the proof (the money the business is already leaking, the $540/mo bakery cut, "the saving covers the fee," built "with the same AI the big players run"). The discipline is fixed: empowerment is the PROMISE, the leak number is the PROOF, never reversed. The empowerment brand line lives on the homepage and warm conversations only; the literal "we find the money your business is leaking" cold-open stays on `/savings`, `/sheets/<slug>`, and the standalone demo sites, where abstraction would drop conversion. The audience wedge is unchanged (established, reputation-rich local businesses). Decided via a vault-grounded, market-researched, four-lens advisor pressure-test (April Dunford, internal CEO, Rob Fitzpatrick, Dario). Same session: TSD's first industry-specialized tool (a Ventures-housed product bet, NOT yet built) will be an auto-repair "declined-work win-back" engine that turns a shop's own declined-line history into timed, personalized rebooking texts and a provable recovered-revenue number; the floated real-estate move-prediction idea was evaluated against the incumbents (SmartZip, Offrs, Revaluate, Catalyze) and parked as off-brand and over-crowded. Code touched: `src/pages/Home.jsx` (hero H1 + subhead, and the homepage "Why we do this" thesis beats reframed to the three empowerment-credibility differentiators: we price the leak in real dollars before you pay, we run it so it is a capability not a chore, and it is the same current-generation AI the big players run) and `src/pages/ServiceDetail.jsx` (the "What this saves you" money table now opens under an empowerment frame on every SKU page). See §4 + §8.2; dated history below preserved as-is.
...
**Message layer (2026-06-21):** The hero now leads with empowerment, *"We find the leak. You outrun the franchise,"* with the money-saver proof directly beneath it. Same audience and same wedge as above; what changed is the order the value is told in (empowerment is the promise, the dollar leak is the proof). The empowerment line stays on the homepage and warm conversations; the literal leak cold-open holds on `/savings`, `/sheets`, and the demo sites. See the 2026-06-21 top note.
...
| News / Field Notes (content + social proof) | Live (added 2026-06-19) | `/news` — a public "here's what we've been building" journal; the first post recaps shipped work with anonymized client references. A standing surface for prospects and the company-level twin of the per-client monthly value report. Driven by `src/news-data.js`; per-post `BlogPosting` JSON-LD. |
...
- ✅ **Public News / Field Notes page** (2026-06-19; PROJECT_LOG 2026-06-19): a public `/news` index + `/news/:slug` posts, prerendered via `vite-react-ssg`, driven by a `src/news-data.js` catalog; "News" added to nav + footer; per-post `<title>/<meta>` and `Blog` + `BlogPosting` JSON-LD generated from the catalog. First post recaps shipped work with the anonymized client references already on the site. A standing public "here's what we did" surface — the company-level twin of the per-client monthly value report.
