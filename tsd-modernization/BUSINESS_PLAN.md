...
Six named services, each with its own route and prerendered page (`/services` + `src/services-data.js`), plus an add-ons strip (Reviews & reputation, Lead follow-up, Local SEO, workflow automations — still individually selectable estimator line items). Every service page opens with a **"What this saves you"** money table (`savesRows` in `services-data.js`), an anonymized proof line, and links to the `/savings` calculator and its printable `/sheets/<slug>` one-pager. Old bucket slugs 301: `ai-integration` → `front-desk`, `process-modernization` → `booking-bridge`; `/ai-receptionist` → `/services/front-desk`.
...
### 5.3 TSD Booking Bridge (`/services/booking-bridge`)
One booking front door wired to the calendar the client already runs — confirmations, reminders, lead routing automated on tools they already pay for. Saves-pitch: phone tag, double-bookings, and no-shows. Timeline: 1–3 weeks.
...
### 5.5 TSD Lead Engine (`/services/lead-engine`) — new 2026-06-12
Conversion-built landing funnel + a lead dashboard the client's team actually works from (statuses, notes, ownership, automated follow-up, spam protection, outcome analytics; multilingual when the market needs it). Saves-pitch: stops referrals and ad clicks dying in an unread inbox. Proof: live for a Carolina insurance agency. Timeline: 2–3 weeks.

### 5.6 TSD Cost-Cut Audit (`/services/cost-cut-audit`) — new 2026-06-12
Line-by-line teardown of software, subscription, and vendor bills: overlap map, kill list with dollar amounts, switch plan + migration help, 30-day check-in. **Flat fee quoted on the fit call; guaranteed — if the audit doesn't find at least its fee in annual savings, it's free** (fee amount pending Nash's sign-off). Proof: $540/mo found at a local bakery (website vendor + domain middleman). Supersedes the scope-and-recommend stepping stone as the official low-risk entry offer. Timeline: one week.
